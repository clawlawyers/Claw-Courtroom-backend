...
const authenticateUser = (req, res, next) => {
  req.user = {
    id: "courtroom-dev", // Replace this with actual user ID from your auth system
  };
  next();
};

// Route to upload file and return public URL
router.post(
  "/uploadFileWithLink",
  authenticateUser,
  upload.single("file"),
  async (req, res) => {
    const userId = req.user.id; // Assuming user ID is in the request after authentication
    const file = req.file;
    ...
    // Path to store the file in Google Cloud Storage (e.g., userId/filename)
    ...
    try {
      ...
      const blobStream = blob.createWriteStream({
        ...
        contentType: file.mimetype, // Set the content type based on file mime type
      });
      ...
      blobStream.on("finish", async () => {
        ...
        res.status(200).send({
          ...
          publicUrl: publicUrl, // Public URL of the file
        });
      });

      // End the stream and upload the file to GCS
